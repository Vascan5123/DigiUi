...
app.initializers.add("vascan/digi-ui", () => {
  ...
  extend(IndexPage.prototype, "viewItems", function (items) {
    ...
    console.log();
    items.add("jumbotron", <div class="jumbotron_class"></div>);
    // Добавляем текст сверху
    items.add(
      "Text_title_center_block",
      <p class="Text_title_center_block_class">
        <span> {app.translator.trans("digi-ui.forum.what_is_new")}</span>
      </p>
    );
  });
  extend(DiscussionListItem.prototype, "view", function (view) {
    ...
    try {
      if (
        ...
        app.current.matches(DiscussionsUserPage)
      ) {
        ...
        elementClassName =
          view.children[2].children[0].attrs.className.split(" ");
        elementClassName[0] += "--grid";
        view.children[2].children[0].attrs.className =
          elementClassName.join(" ");

        // Modificare clasa DiscussionListItem-main
        elementClassName =
          view.children[2].children[2].attrs.className.split(" ");
        elementClassName[0] += "--grid";
        view.children[2].children[2].attrs.className =
          elementClassName.join(" ");

        // Modificare clasa DiscussionListItem-title
        elementClassName =
          view.children[2].children[2].children[0].attrs.className.split(" ");
        elementClassName[0] += "--grid";
        view.children[2].children[2].children[0].attrs.className =
          elementClassName.join(" ");
      }
    } catch {}
  });
  ...
  override(
    require("@fof-upload").components.FileManagerButton.prototype,
    "view",
    function (original) {
  ...
  extend(ComposerBody.prototype, "view", function (view) {
    view.children[0].children[1].children[1].children[0].attrs.class1 =
  ...
  function textEditorF() {
    if (app.current.matches(IndexPage) && window.innerWidth > "425") {
      let text = document.getElementsByClassName("textarea_create_post")[0]
        .value;
      if (text != undefined) {
        if (text.length > 0) {
          document
            .getElementsByClassName("fof-upload-button")[0]
            .classList.remove("big_upload_icon");
        } else {
          document
            .getElementsByClassName("fof-upload-button")[0]
            .classList.add("big_upload_icon");
        }
      } else {
        document
          .getElementsByClassName("fof-upload-button")[0]
